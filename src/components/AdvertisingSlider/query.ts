...
export const GET_NFT_OFFERS = gql`
  query collectionItems($tokenId: BigInt! $tokenAddress: String!) {
    buyingOffers(where:{tokenId:$tokenId,tokenAdress:$tokenAddress,offerStatus:ACTIVE }) {
      id
      listingId
      newOfferedPrice
    }
    stakingOffers(where:{tokenId:$tokenId,tokenAdress:$tokenAddress,offerStatus:ACTIVE }) {
      id
      stakingId
